...
// Skipped on mobile: WebGL performance and stability varies a lot across
// phone GPUs/browsers, and the animated background isn't worth the risk
// of jank or glitches on a small screen. Falls back to the plain black
// background, which still looks intentional and on-brand.
(function initThree() {
...
// Wrapped in a function so post.html can call it again after it
// dynamically injects the article content and TOC.
window.initPostObservers = function() {
...
// ═══════════════════════════════════════════
// AUTO BLOG LOADER
// Fetches posts-data.json (edited via the CMS at /admin) and
// renders cards on blog.html, the homepage preview, and the
// related-posts section of post.html — no manual file editing.
// ═══════════════════════════════════════════
(async function renderBlogCards() {
  ...
  if (!blogGrid && !previewGrid) return; // not a page that needs this
  ...
  try {
    ...
    console.warn('Could not load posts-data.json', e);
    return;
  }

  const sorted = [...posts].sort((a, b) => new Date(b.date) - new Date(a.date));
  const link = p => `post.html?slug=${p.slug}`;
  ...
  if (blogGrid) {
    if (!sorted.length) {
      ...
      let html = `<div class="blog-featured" style="display:grid;grid-template-columns:1fr 1fr;gap:2px;margin-bottom:2px">
        <a href="${link(first)}" class="blog-card lg">
          <div class="bc-arrow">&nearr;</div>
          <div class="bc-cat">${first.category}</div>
          <div class="bc-title">${first.title}</div>
          <p class="bc-excerpt">${first.description || ''}</p>
          <div class="bc-meta"><span>${formatDate(first.date)}</span><span>${first.read_time}</span></div>
        </a>`;
      if (rest[0]) {
        html += `<a href="${link(rest[0])}" class="blog-card md">
          <div class="bc-arrow">&nearr;</div>
          <div class="bc-cat">${rest[0].category}</div>
          <div class="bc-title">${rest[0].title}</div>
          <p class="bc-excerpt">${rest[0].description || ''}</p>
          <div class="bc-meta"><span>${formatDate(rest[0].date)}</span><span>${rest[0].read_time}</span></div>
        </a>`;
      }
      html += `</div><div class="blog-list" style="display:grid;grid-template-columns:repeat(3,1fr);gap:2px">`;
      rest.slice(1).forEach(p => {
        html += `<a href="${link(p)}" class="blog-card sm">
          <div class="bc-arrow">&nearr;</div>
          <div class="bc-cat">${p.category}</div>
          <div class="bc-title">${p.title}</div>
          <div class="bc-meta"><span>${formatDate(p.date)}</span><span>${p.read_time}</span></div>
        </a>`;
      });
      html += `</div>`;
  ...
  if (previewGrid) {
    ...
    previewGrid.innerHTML = latest.map((p, i) => `
      <a href="${link(p)}" class="blog-card${i === 0 ? ' featured' : ''} reveal" style="transition-delay:${i * 0.1}s">
        <div class="bc-arrow">&nearr;</div>
        <div class="bc-cat">${p.category}</div>
        <div class="bc-title">${p.title}</div>
        <div class="bc-meta"><span>${formatDate(p.date)}</span><span>${p.read_time}</span></div>
